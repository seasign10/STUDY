...
#avg_score = total_score / len(socres) => length은 길이. 해당 키의 갯수만큼 설정 됨.
print(avg_score)
...
데이타 프렉티스원, 앱파이 로또 리절트 체크 

잘못된거 수정하고 커밋하기



텔레그램에 api요청을 보내서 연습으로 나눔로또 페이지를 이요앻서 어떤 정보를 주는지 조작할수있다.

내일 : 텔레그램 챗봇 완성 (따라치기만하면됨 왜냐하면 말도안되는 이론이 ...많다 오타없이 잘 따라오면됨.)
